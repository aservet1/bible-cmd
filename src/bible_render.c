...
static void
bible_output_verse(const bible_verse *verse, FILE *f, const bible_config *config)
{
    fprintf(
        f,
        config->highlighting ?
...
static bool
bible_output(const bible_ref *ref, FILE *f, const bible_config *config)
{
    bible_next_data next = {
        .current = 0,
        .next_match = -1,
        .matches = {
            {-1, -1},
    ...
    bible_verse *last_printed = NULL;
    for (int verse_id; (verse_id = bible_next_verse(ref, config, &next)) != -1; ) {
        bible_verse *verse = &bible_verses[verse_id];
        bible_book *book = &bible_books[verse->book - 1];
        ...
        if (config->pretty) {
            ...
            bible_output_verse(verse, f, config);
        } else {
            fprintf(
                f,
                config->highlighting ?
...
static int
bible_render_pretty(const bible_ref *ref, const bible_config *config)
{
    int fds[2];
    if (pipe(fds) == -1) {
        return 1;
    ...
    bool printed = bible_output(ref, output, config);
    if (!printed) {
        kill(pid, SIGTERM);
    }
    fclose(output);
...
int
bible_render(const bible_ref *ref, const bible_config *config)
{
    if (config->pretty) {
        return bible_render_pretty(ref, config);
    }
    bible_output(ref, stdout, config);
    return 0;
}
