...
bible_newref()
{
    return calloc(1, sizeof(bible_ref));
}
...
static bool
bible_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
bible_book_matches(const bible_book *book, const char *s)
{
    return bible_bookequal(book->name, s, false) ||
        bible_bookequal(book->abbr, s, false) ||
        bible_bookequal(book->name, s, true);
}

static int
bible_book_fromname(const char *s)
{
    for (int i = 0; i < bible_books_length; i++) {
        const bible_book *book = &bible_books[i];
        if (bible_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
bible_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
bible_parseref(bible_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (bible_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = bible_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
