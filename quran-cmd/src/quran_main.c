...
const char *
usage = "usage: bible [flags] [reference...]\n"
    "\n"
    "Flags:\n"
...
main(int argc, char *argv[])
{
    ...
    for (int opt; (opt = getopt(argc, argv, "A:B:CeplWh")) != -1; ) {
        ...
        case 'A':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "bible: invalid flag value for -A\n\n%s", usage);
                return 1;
            }
            break;
        case 'B':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "bible: invalid flag value for -B\n\n%s", usage);
                return 1;
            }
            break;
        case 'C':
        ...
        case 'h':
            ...
            return 0;
        case '?':
            fprintf(stderr, "bible: invalid flag -%c\n\n%s", optopt, usage);
            return 1;
        }
    }
    ...
    if (argc == optind) {
        using_history();
        while (true) {
            char *input = readline("bible> ");
            if (input == NULL) {
                break;
            }
            add_history(input);
            bible_ref *ref = bible_newref();
            int success = bible_parseref(ref, input);
            free(input);
            if (success == 0) {
                bible_render(ref, &config);
            }
            bible_freeref(ref);
        }
    } else {
        ...
        bible_ref *ref = bible_newref();
        int success = bible_parseref(ref, ref_str);
        free(ref_str);
        if (success == 0) {
            bible_render(ref, &config);
        }
